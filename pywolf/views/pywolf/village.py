...
def village(request, village_no, day_no):
    ...
    if now > update_datetime:
        if prg.village_status != VillageStatus.END and prg.village_status != VillageStatus.ABOLITION:  # 終了か廃村していなければ
            # 更新実行して、そのままログ取得＆表示へ
    ...
    e_form = False
    if progress.village_status == VillageStatus.PROLOGUE and login_info['login_id'] and not login_participant:
    ...
    return render(request, "pywolf/village.html",
                  {'village': village,                       # 村情報
                   ...
                   'VillageStatus': VillageStatus,          # 村ステータスEnum
                   'voices': voices,                         # 発言
